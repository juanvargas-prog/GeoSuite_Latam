def mexico_QueryValplacas(placas):
    query = """
    ...
    UPDATE """ + placas + """ SET nom_loc = regexp_replace(nom_loc, '[\\|\\!"#@$%&/()¿¡¨{}\\[\\]._:,;+\\-*=/¬°]', ' ', 'g')
    WHERE nom_loc ~ '[\\|\\!"#@$%&/()¿¡¨{}\\[\\]._:,;+\\-*=/¬°]';
    ...
    UPDATE """ + placas + """ SET nom_loc = TRIM(nom_loc) WHERE nom_loc <> TRIM(nom_loc);
    ...
    UPDATE """ + placas + """ SET nom_loc = REPLACE(nom_loc,'  ',' ') WHERE nom_loc LIKE '%  %';
    ...
    UPDATE """ + placas + """ SET nom_loc = UPPER(nom_loc) WHERE nom_loc <> UPPER(nom_loc);
...
def mexico_pl_incidencia(mv_mexico, placas_mexico, predios_mexico, texto_filtro, key,idx):
    ...
    if key ==1:
        ...
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[1\]', na=False),'id'])
        ...
        def marcar_simbolos(row):
            if row['nomvia'] != 'S/N':
                columnas = ['nom_estado', 'nom_mun', 'nom_loc', 'nom_urb', 'tipovia', 'nomvia', 'direccion']
                for col in columnas:
        ...
        placas_mexico['incidencia'] = placas_mexico.apply(marcar_simbolos, axis=1)   
        
    if key ==2:
        #2 PLACAS ID_CAPA REPETIDO
        frecuencias = placas_mexico['id_capa'].value_counts().reset_index()
        valor_dict = frecuencias[frecuencias['count'] > 1].set_index('id_capa')['count'].to_dict()
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[2\]', na=False),'id'])
        placas_mexico['incidencia'] = placas_mexico.apply(
    ...
    if key ==3:
        #3 PLACAS REFIJO
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[3\]', na=False),'id'])
        
        filtro = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & (placas_mexico['tipovia'].isna())]
        resultados=[]
        ...
        for row in resultados_df.itertuples(index=False):
            if placas_mexico.at[row[0],'id'] in id_excp:
                continue
            if not (mv_mexico.at[row[1], 'tipovia']==None):
                tiponulo.append(row[0])
                
        cond1 = placas_mexico.index.isin(tiponulo)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['exclusion'].str.contains(r'\[3\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 3. Tipovia vacio"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "3. Tipovia vacio"   
    
    if key ==4:
        #4 VIA VACIA
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[4\]', na=False),'id'])
                
        filtro = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & ((placas_mexico['nomvia'].isna()) | (placas_mexico['nomvia'] == 'S/N'))]
        resultados=[]
        for row in filtro.itertuples():
            candidates = list(idx.nearest(row.geom.bounds, num_results=1))
            resultados.append([row.Index,candidates[0]])
        ...
        for row in resultados_df.itertuples(index=False):
            if placas_mexico.at[row[0],'id'] in id_excp:
                continue
            if mv_mexico.at[row[1], 'nomvia'] is not None and mv_mexico.at[row[1], 'nomvia'] != 'S/N':
                nomnulo.append(row[0])
                
        cond1 = placas_mexico.index.isin(nomnulo)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['exclusion'].str.contains(r'\[4\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 4. nombre via vacio"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "4. Nombre via vacio"   
        
        
    if key ==5:
        #5 generadora VACIA
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[5\]', na=False),'id'])
                
        filtro = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & (placas_mexico['generadora'].isna())]
        resultados=[]
        for row in filtro.itertuples():
            candidates = list(idx.nearest(row.geom.bounds, num_results=1))
            resultados.append([row.Index,candidates[0]])
        ...
        for row in resultados_df.itertuples(index=False):
            if placas_mexico.at[row[0],'id'] in id_excp:
                continue
            if not (mv_mexico.at[row[1], 'generadora']==None):
                gennula.append(row[0])
                
        cond1 = placas_mexico.index.isin(gennula)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['exclusion'].str.contains(r'\[5\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 5. Generadora vacia"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "5. Generadora vacia"   

    if key ==6:  
        #6 PLACAS VS MAVVIAL    
        id_excp = list(placas_mexico.loc[placas_mexico['exclusion'].str.contains(r'\[6\]', na=False),'id']) 
                
        filtro = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)]
        resultados=[]
        for row in filtro.itertuples():
        ...
        for row in resultados_df.itertuples(index=False):
            if placas_mexico.at[row[0],'id'] in id_excp:
                continue
            if not ((placas_mexico.at[row[0], 'tipovia']==mv_mexico.at[row[1], 'tipovia']) and (placas_mexico.at[row[0], 'nomvia']==mv_mexico.at[row[1], 'nomvia']) and (placas_mexico.at[row[0], 'generadora']==mv_mexico.at[row[1], 'generadora'])):
                if not ((placas_mexico.at[row[0], 'tipovia']==mv_mexico.at[row[2], 'tipovia']) and (placas_mexico.at[row[0], 'nomvia']==mv_mexico.at[row[2], 'nomvia']) and (placas_mexico.at[row[0], 'generadora']==mv_mexico.at[row[2], 'generadora'])):
                    if placas_mexico.at[row[0], 'incidencia'] is None:
                        placas_mexico.at[row[0], 'incidencia']='6. No corresponde la placa con el nombre de via'
                    else:
                        placas_mexico.at[row[0], 'incidencia']=placas_mexico.at[row[0], 'incidencia'] + ' - 6. No corresponde direccion con el de la via'

    if key ==7:   
        # 7 NUM PUERTA REPETIDO
        agrupados = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)][['cod_loc','tipovia','nomvia','generadora']].dropna().drop_duplicates().reset_index(drop=True)
        copia_placas=placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)].dropna(subset=['cod_estado', 'cod_mun','cod_loc','tipovia','nomvia','generadora']).reset_index(drop=False).drop(['cod_estado', 'cod_mun'], axis=1).set_index(['cod_loc','tipovia','nomvia','generadora']).sort_index()
        repetidos_total=[]
        ...
        cond1 = placas_mexico.index.isin(repetidos_total)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['exclusion'].str.contains(r'\[7\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 7. Numero de puerta repetido"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "7. Numero de puerta repetido"   
    ...
    if key ==8: 
        ...
        for i, geometry in enumerate(predios_mexico.geometry):
            if geometry is not None:
                idx_pr.insert(i, geometry.bounds)
                    
        agrupados = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)][['cod_loc','tipovia','nomvia','generadora','placa']].dropna().drop('placa', axis=1).drop_duplicates().reset_index(drop=True)
        copia_placas=placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)][['cod_loc','tipovia','nomvia','generadora','placa','geom']].dropna().reset_index(drop=False).set_index(['cod_loc','tipovia','nomvia','generadora']).sort_index()
        ...
        for row in agrupados.itertuples(index=False):
            ...
            if len(pares)>1:
                ...
                for actual, siguiente in zip(pares, pares[1:]):
                    ...
                    for i in candidates:
                        if linea.intersects(mv_mexico.geometry[i]):
                    ...
                    if not Error:
                        ...
                        cruce=0
                        for i in candidates:
                            if linea.intersects(predios_mexico.geometry[i]):
                                cruce += 1
                            if cruce == 3:
                                errores_total.extend(list(filtro.loc[[actual,siguiente],'index']))
                                break
        ...
        cond1 = placas_mexico.index.isin(errores_total)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['incidencia'].str.contains(r'\[8\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 8. Error costados"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "8. Error costados"

        #MARCAR ERRORES SALTOS
        saltos_total=list(set(saltos_total))
        cond1 = placas_mexico.index.isin(saltos_total)
        cond2 = placas_mexico['incidencia'].isna()
        cond3 = placas_mexico['incidencia'].str.contains(r'\[9\]', na=False)
        placas_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = placas_mexico['incidencia'] + " - 9. Salto de placa"
        placas_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "9. Salto de placa"
    ...
    if key == 10:
                ...
                if dist2<dist1:
                    ...
                    if (comp_p1>comp_p2):
                        if (lista[i+1][1]-lista[i][1])<dist2:
                            ...
                            lista[i+1][2] = placas_mexico.loc[lista[i+1][0],'geom'] = Point(lista[i][2].x+emas,lista[i][2].y+nmas)
                            ...
                            placas_mexico.loc[lista[i+1][0],'geom'] = placas_mexico.loc[lista[i+2][0],'geom']
                            placas_mexico.loc[lista[i+2][0],'geom'] = lista[i+1][2]
                            lista[i+1][2] = placas_mexico.loc[lista[i+1][0],'geom']
                            lista[i+2][2] = placas_mexico.loc[lista[i+2][0],'geom']
                            edicion.add(lista[i+1][0])
                        ...
                        if (lista[i+2][1]-lista[i][1])>dist1:
                            ...
                            lista[i+2][2] = placas_mexico.loc[lista[i+2][0],'geom'] = Point(lista[i+1][2].x+emas,lista[i+1][2].y+nmas)
                            edicion.add(lista[i+2][0])
                        else:
                            if lista [i+1][3] is None:
                                coordenadas.append([lista[i+1][0],str(geom_original.loc[lista[i+1][0],'geom'].wkt)])
        ...
        #Hallar agrupados
        geom_original=placas_mexico[['geom']].copy()
        placas_mexico=placas_mexico.to_crs(epsg=3857)
        copia_placas=placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & (placas_mexico['exclusion'].isna())].dropna(subset=['cod_loc','nomvtotal','generadora','placa']).reset_index(drop=False).set_index(['cod_loc','nomvtotal','generadora']).sort_index()[['index','placa','geom','coord_ini']]#.to_crs(epsg=3857)
        copia_placas['placa'] = copia_placas['placa'].apply(lambda x: int(re.sub(r'\D', '', x)))
        edicion = set()
        coordenadas = []
        agrupados = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & (~placas_mexico['nomvtotal'].str.contains('S/N', na=False)) ][['cod_loc','nomvtotal','generadora','placa']].dropna().drop('placa', axis=1).drop_duplicates()
        for row in agrupados.itertuples(index=False):
        ...
        placas_mexico=placas_mexico.to_crs(epsg=4326)

        #TRAER  COORDENADAS
        df = pd.DataFrame(coordenadas, columns=['idx', 'coorde'])
        merge = placas_mexico.join(df.set_index('idx'))
        merge['coord_ini'] = merge['coord_ini'].fillna(merge['coorde'])
        placas_mexico = merge.drop(columns=['coorde'])

        #MARCAR 
        cond1 = placas_mexico.index.isin(edicion)
        cond2 = placas_mexico['observ_pos'].isna()
        placas_mexico.loc[cond1 & ~cond2, 'observ_pos'] = placas_mexico['observ_pos'] + " - Posición modificada"
        placas_mexico.loc[cond1 & cond2, 'observ_pos'] = "Posición modificada"

        #Marca devolucion
        placas_mexico['edicion10']=None
        placas_mexico.loc[cond1,'edicion10'] = 'SI'
    return placas_mexico

def mexico_validar_placas(engine, mavvial, placas, predios, texto_filtro, seleccionados):
    ...
    campos_1 = 'id,geom,nom_estado,nom_mun,nom_loc,nom_urb,tipovia,nomvia,direccion,atipico'
    ...
    campos_6 = 'id,geom,cod_estado,cod_mun,tipovia,nomvia,generadora,placa,atipico'
    campos_7 = 'id,geom,cod_estado,cod_mun,cod_loc,tipovia,nomvia,generadora,placa,atipico'
    campos_8 = 'id,geom,cod_estado,cod_mun,cod_loc,tipovia,nomvia,generadora,placa,atipico'
    campos_10 = 'id,geom,cod_estado,cod_mun,cod_loc,nomvtotal,generadora,placa,atipico,observ_pos,coord_ini'
    ...
    placas_mexico = gpd.read_postgis(sql, engine, geom_col='geom',crs= 'EPSG:4326')
    placas_mexico['incidencia'] = None
    placas_mexico['exclusion'] = placas_mexico['atipico'].apply(
    ...
    if (set(seleccionados) - {1,2,7,10}) != set():#Que no
        #TRAER MALLA
        sql=f"SELECT id, geom, tipovia, nomvia, generadora FROM {mavvial} WHERE cod_estado = '{dep}' AND cod_mun = '{prov}';"
        ...
        if ({3,4,5,6,8} & set(seleccionados)) != set():#Que si
            ...
            for i, geometry in enumerate(mv_mexico.geometry):
                if geometry is not None:
    ...
    #Traer predios_mexico
    if ({8} & set(seleccionados)) != set():#Que si
        #TRAER PREDIOS
        sql=f"SELECT id, geom FROM {predios};"
        predios_mexico = gpd.read_postgis(sql, engine, geom_col='geom',crs= 'EPSG:4326')
        
    else:
        predios_mexico=''
    
    for key in seleccionados:
        placas_mexico = mexico_pl_incidencia(mv_mexico, placas_mexico, predios_mexico, texto_filtro, key,idx)
        
    if 10 in seleccionados:
        if set(seleccionados) == {10}:
            df_temporal = placas_mexico[placas_mexico['edicion10'].notna()][['id','observ_pos','coord_ini','geom']]
        else:
            df_temporal = placas_mexico[placas_mexico['edicion10'].notna() | placas_mexico['incidencia'].notna()][['id','incidencia','observ_pos','coord_ini','geom']]
        df_temporal.to_postgis('tabla_temporal', con=engine, index=False, if_exists='replace')
    else:
        df_temporal = placas_mexico[placas_mexico['incidencia'].notna()][['id', 'incidencia']]
