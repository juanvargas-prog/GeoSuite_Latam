def mexico_QueryValMavvial(mavvial,placas):
    query = """
    ...
    UPDATE """ + mavvial + """ SET nom_loc = UPPER(nom_loc) WHERE nom_loc <> UPPER(nom_loc);
    ...
    UPDATE """ + mavvial + """ SET nom_loc = regexp_replace(nom_loc, '[\\|\\!"#@$%&/()¿¡¨{}\\[\\]._:,;+\\-*=/¬°]', ' ', 'g')
    WHERE nom_loc ~ '[\\|\\!"#@$%&/()¿¡¨{}\\[\\]._:,;+\\-*=/¬°]';
    ...
    UPDATE """ + mavvial + """ SET nom_loc = TRIM(nom_loc) WHERE nom_loc <> TRIM(nom_loc);
    ...
    UPDATE """ + mavvial + """ SET nom_loc = REPLACE(nom_loc,'  ',' ') WHERE nom_loc LIKE '%  %';
...
def mexico_mv_incidencia(mv_mexico, placas_mexico, texto_filtro, key,idx):

    mifiltro=texto_filtro #####AQUI VA EL FILTRO#####
    partes = mifiltro.split(',')
    dep=partes[0]
    prov=partes[1]
    print("FIltrooo",mifiltro)
    if key ==1 :

        # 1. Validar registro con caracteres
        id_excp = list(mv_mexico.loc[mv_mexico['exclusion'].str.contains(r'\[1\]', na=False),'id'])
        ...
        def marcar_simbolos(row):
            if row['nomvia'] != 'S/N':
                columnas = ['nom_estado', 'nom_mun', 'nom_loc', 'tipovia', 'nomvia', 'nomvtotal']
                for col in columnas:
        ...
        mv_mexico['incidencia'] = mv_mexico.apply(marcar_simbolos, axis=1)
    ...
    if key == 2:
        ...
        id_excp = list(mv_mexico.loc[mv_mexico['exclusion'].str.contains(r'\[2\]', na=False),'id'])
        mv_mexico['incidencia'] = mv_mexico.apply(
    ...
    if key == 3:        
        filtro = mv_mexico[(mv_mexico['cod_estado']==dep) & (mv_mexico['cod_mun']==prov)].copy()
        ...
        #----------------------
        def es_nomvia_vacio_o_sn(valor):
            if pd.isna(valor):  # detecta NaN o None reales
                return True
            valor = str(valor).strip().upper()
            return valor in ['', 'S/N', 'SN', 'SIN NOMBRE', 'SIN VIA', 'NO DISPONIBLE', 'NULL', 'N/D', 'NA']


        paralelas_nombres = joined[(joined['relacion'] == 'Paralelas') | (joined['relacion'] == 'Paralelas invertidas')]
        SN = paralelas_nombres[
            paralelas_nombres['nomvia_1'].apply(es_nomvia_vacio_o_sn) &
            (paralelas_nombres['longitud_1'] > 15)
        ]

        id_par_sn = SN[(SN['nomvia_2'].notna()) & ~(SN['nomvia_2'].str.contains('S/N',na=False))].index.unique()
        id_cll = joined[
            ((joined['nomvia_1'].isna()) | (joined['nomvia_1'].str.contains('S/N', na=False))) &
            (joined['validacion_call'] == 0)
        ].index.unique()

        idnulo=set(id_par_sn)-set(id_cll)

        cond1 = mv_mexico.index.isin(idnulo)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[3\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 3. nomvia vacio"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "3. nomvia vacio"
    ...
    if key == 4:    
        ...
        joined['relacion'] = joined['diferencia_orientacion'].apply(clasificar_relacion)
        saltos_cuadras=[]
        def limpiar_generadora(val):
            if pd.isna(val):
                return np.nan
            val_str = str(val).strip().upper()
            if val_str in ['S/N', 'SN', '', 'SIN NOMBRE', 'SIN VIA', 'NO DISPONIBLE', 'NULL', 'N/D', 'NA', 'N/A']:
                return np.nan
            return val

        joined['generadora_1'] = joined['generadora_1'].apply(limpiar_generadora)
        joined['generadora_2'] = joined['generadora_2'].apply(limpiar_generadora)
        ...
        paralelas_nombres = joined[(joined['relacion'] == 'Paralelas') | (joined['relacion'] == 'Paralelas invertidas')]
        SN = paralelas_nombres[
            paralelas_nombres['generadora_1'].apply(es_nomvia_vacio_o_sn) &
            (paralelas_nombres['longitud_1'] > 15)
        ]

        id_par_sn = SN[
            SN['generadora_2'].notna() &
            ~(SN['generadora_2'].astype(str).str.contains('S/N', na=False))
        ].index.unique()

        id_cll = joined[
            ((joined['generadora_1'].isna()) | (joined['generadora_1'].str.contains('S/N', na=False))) &
            (joined['validacion_call'] == 0)
        ].index.unique()
        ...
        cond1 = mv_mexico.index.isin(idnulo)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[4\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 4. Generadora vacia"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "4. Generadora vacia"

    if key == 5:     
        #5 MV REPETIDAS

        #P1
        copia_malla=mv_mexico[(mv_mexico['cod_estado']==dep) & (mv_mexico['cod_mun']==prov) &(~mv_mexico['nomvtotal'].str.contains('S/N', na=False))].dropna(subset=['cod_estado', 'cod_mun','cod_loc','nomvtotal','generadora']).reset_index(drop=False).set_index(['cod_estado','cod_mun','cod_loc','nomvtotal','generadora']).sort_index()
        #Agregarlongitud
        ...
        #P2
        filtro = mv_mexico[(mv_mexico['cod_estado']==dep) & (mv_mexico['cod_mun']==prov) & (~mv_mexico['nomvtotal'].str.contains('S/N', na=False))].copy()
        ...
        mismo_nomvtotal = joined['nomvtotal_1'] == joined['nomvtotal_2']

        gen1_nan = joined['generadora_1'].isna()
        gen2_nan = joined['generadora_2'].isna()

        generadoras_iguales = (joined['generadora_1'] == joined['generadora_2']) | (gen1_nan & gen2_nan)

        vias_rep = joined[mismo_nomvtotal & generadoras_iguales].copy()
        vias_rep = vias_rep[vias_rep['validacion_t'] != 2]
        vias_rep_con_t = vias_rep[(vias_rep['validacion_t'] == 2)]

        ind_rep_p2 = vias_rep_con_t.index
        ind_rep = set(ind_rep_p1) - set(ind_rep_p2)

        cond1 = mv_mexico.index.isin(ind_rep)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[5\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 5. Via repetida"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "5. Via repetida"
    ...
    if key == 6:     
        ...
        #Lista de exclusion
        id_excp = list(mv_mexico.loc[mv_mexico['exclusion'].str.contains(r'\[6\]', na=False),'id'])
        # Función para obtener el punto inicial de una geometría, sea LineString o MultiLineString
        def get_start_point(geom):
            if isinstance(geom, LineString):
        ...
        condicion = preprecondicion[~((preprecondicion['nomvtotal_1'] != preprecondicion['nomvtotal_2']) & (preprecondicion['cod_loc_1'] != preprecondicion['cod_loc_2']))].copy()####
        ...
        mv_mexico_actualizado = mv_mexico.merge(condicion[['id_1', 'incidencia_1']], left_on='id', right_on='id_1', how='left')
        mv_mexico_actualizado['incidencia'] = mv_mexico_actualizado['incidencia_1'].combine_first(mv_mexico_actualizado['incidencia'])
        mv_mexico_actualizado = mv_mexico_actualizado.drop(columns=['id_1', 'incidencia_1'])
        mv_mexico = mv_mexico_actualizado.copy()
    ...
    if key == 7: 
        ...
        #MARCAR 
        cond1 = mv_mexico.index.isin(saltos_gen)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[7\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 7. Salto generadora"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "7. Salto generadora"
    ...
    if key == 8:
        ...
        cond1 = mv_mexico.index.isin(errores_apuntamiento)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[8\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 8. Error apuntamiento"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "8. Error apuntamiento"
    ...
    if key == 9 :
        ...
        filtro = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov)].dropna(subset=['nomvtotal','generadora'])
        resultados=[]
        ...
        for row in resultados_df.itertuples(index=False):
            if ((placas_mexico.at[row[0], 'nomvtotal']==copia_malla.at[row[1], 'nomvtotal']) and (placas_mexico.at[row[0], 'generadora'] in copia_malla.at[row[1], 'list_generadora'])):
                combo.append([row[1],row[0],placas_mexico.at[row[0],'placa']])
            elif ((placas_mexico.at[row[0], 'nomvtotal']==copia_malla.at[row[2], 'nomvtotal']) and (placas_mexico.at[row[0], 'generadora'] in copia_malla.at[row[2], 'list_generadora'])):
                combo.append([row[2],row[0],placas_mexico.at[row[0],'placa']])
        ...
        for row in impar_fin.itertuples(index = False):
            az_mv = calcular_azimuth(get_start_point(mv_mexico.loc[row[0],'geom']),get_end_point(mv_mexico.loc[row[0],'geom']))
            az_mv = radians((degrees(az_mv)+360)%360)
            az_p = calcular_azimuth(placas_mexico.loc[row[2],'geom'],placas_mexico.loc[row[4],'geom'])
        ...
        for row in par_fin.itertuples(index = False):
            ...
            if abs(az_mv-az_p)>radians(100):
                mala_orientacion.add(row[0])
        #MARCAR 
        cond1 = mv_mexico.index.isin(mala_orientacion)
        cond2 = mv_mexico['incidencia'].isna()
        cond3 = mv_mexico['exclusion'].str.contains(r'\[9\]', na=False)
        mv_mexico.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_mexico['incidencia'] + " - 9. Invertir vector"
        mv_mexico.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "9. Invertir vector"

        
    if key ==10 :
        #10 RANGO

        agrupados = mv_mexico[(mv_mexico['cod_estado']==dep) & (mv_mexico['cod_mun']==prov) & (~mv_mexico['nomvia'].str.contains('S/N', na=False))][['cod_loc','tipovia','nomvia','generadora']].dropna().drop_duplicates()
        copia_placas = placas_mexico[(placas_mexico['cod_estado']==dep) & (placas_mexico['cod_mun']==prov) & (~placas_mexico['nomvia'].str.contains('S/N', na=False)) & (placas_mexico['atipico'].isna())][['cod_loc','tipovia','nomvia','generadora','placa']].dropna().set_index(['cod_loc','tipovia','nomvia','generadora']).sort_index()
        sin_ran = set()
        sin_par = set()
        sin_impar = set()
        for row in agrupados.itertuples():
            ...
            if len(pares)>0:
                mv_mexico.at[row[0],'rangopar']=f"{pares[0]}, {pares[-1]}"
            else:
                sin_par.add(row[0])
            if len(impares)>0:
                mv_mexico.at[row[0],'rangoimpar']=f"{impares[0]},{impares[-1]}"
            else:
                sin_impar.add(row[0])
        #MARCAR 
        cond1 = mv_mexico.index.isin(sin_ran|sin_par)
        mv_mexico.loc[cond1 , 'rangopar'] = "Sin rango par"

        #MARCAR 
        cond1 = mv_mexico.index.isin(sin_ran|sin_impar)
        mv_mexico.loc[cond1 , 'rangoimpar'] = "Sin rango impar"
    
    return mv_mexico
    
def mexico_validar_mv(engine, mavvial, placas, texto_filtro, seleccionados):
    ...
    campos_1 = 'id,geom,nom_estado,nom_mun,nom_loc,tipovia,nomvia,nomvtotal,atipico'
    campos_2 = 'id,geom,id_capa,atipico'
    campos_3 = 'id,geom,cod_estado,cod_mun,nomvia,generadora,atipico'
    campos_4 = 'id,geom,cod_estado,cod_mun,generadora,atipico'
    campos_5 = 'id,geom,cod_estado,cod_mun,cod_loc,nomvtotal,generadora,atipico'  
    campos_6 = 'id,geom,cod_estado,cod_mun,cod_loc,nomvtotal,atipico'
    campos_7 = 'id,geom,cod_estado,cod_mun,cod_loc,nomvtotal,generadora,atipico'   
    campos_8 = 'id,geom,cod_estado,cod_mun,cod_loc,nomvtotal,generadora,atipico'
    campos_9 = 'id,geom,cod_estado,cod_mun,cod_loc,tipovia,nomvia,nomvtotal,generadora,atipico'
    campos_10 = 'id,geom,cod_estado,cod_mun,cod_loc,tipovia,nomvia,generadora,atipico'
    ...
    mv_mexico['incidencia'] = None   
    mv_mexico['rangopar'] = None  
    mv_mexico['rangoimpar'] = None  
    mv_mexico['exclusion'] = mv_mexico['atipico'].apply(
    ...
    if (set(seleccionados) - {1,2,3,4,5,6,7,8}) != set():#Que no
        sql=f"SELECT id,geom,cod_estado,cod_mun,cod_loc,tipovia,nomvia,nomvtotal,generadora,placa,atipico FROM {placas} WHERE cod_estado = '{dep}' AND cod_mun = '{prov}';"
    ...
    for key in seleccionados:
        mv_mexico = mexico_mv_incidencia( mv_mexico, placas_mexico, texto_filtro, key,idx)
    
    df_temporal = mv_mexico[['id', 'incidencia', 'rangopar', 'rangoimpar']]
