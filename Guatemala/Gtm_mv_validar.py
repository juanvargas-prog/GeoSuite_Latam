def Gtm_QueryValMavvial(mavvial,placas):
    query = """
...
def Gtm_mv_incidencia(mv_gtm, placas_gtm, texto_filtro, key,idx):
            
    if key ==1 :

        # 1. Validar registro con caracteres
        id_excp = list(mv_gtm.loc[mv_gtm['exclusion'].str.contains(r'\[1\]', na=False),'id'])
        ...
        def marcar_simbolos(row):
            if row['nomvia'] != 'S/N':
                columnas = ['nom_dep', 'nom_mun', 'tipovia', 'nomvia', 'nomvtotal']
                for col in columnas:
        ...
        mv_gtm['incidencia'] = mv_gtm.apply(marcar_simbolos, axis=1)
        
    if key == 2:
        #2 id_capa duplicado
        frecuencias = mv_gtm['id_capa'].value_counts().reset_index()
        valor_dict = frecuencias[frecuencias['count'] > 1].set_index('id_capa')['count'].to_dict()
        id_excp = list(mv_gtm.loc[mv_gtm['exclusion'].str.contains(r'\[2\]', na=False),'id'])
        mv_gtm['incidencia'] = mv_gtm.apply(
    ...
    if key == 3:        
        ...
        filtro = mv_gtm[(mv_gtm['cod_mun']==texto_filtro)].copy()
        ...
        cond1 = mv_gtm.index.isin(idnulo)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[3\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 3. nomvia vacio"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "3. nomvia vacio"
    ...
    if key == 4:    
        ...
        cond1 = mv_gtm.index.isin(generadoranulo)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[4\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 4. Generadora vacia"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "4. Generadora vacia"

    if key == 5:     
        #5 MV REPETIDAS

        #P1
        copia_malla=mv_gtm[(mv_gtm['cod_mun']==texto_filtro) &(~mv_gtm['nomvtotal'].str.contains('S/N', na=False))].dropna(subset=['cod_dep', 'cod_mun','cod_zona','nomvtotal','generadora']).reset_index(drop=False).set_index(['cod_dep','cod_mun','cod_zona','nomvtotal','generadora','costado']).sort_index()
        #Agregarlongitud
        ...
        #P2
        filtro = mv_gtm[(mv_gtm['cod_mun']==texto_filtro) & (~mv_gtm['nomvtotal'].str.contains('S/N', na=False))].copy()
        ...
        cond1 = mv_gtm.index.isin(ind_rep)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[5\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 5. Via repetida"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "5. Via repetida"
    ...
    if key == 6:     
        ...
        #Lista de exclusion
        id_excp = list(mv_gtm.loc[mv_gtm['exclusion'].str.contains(r'\[6\]', na=False),'id'])
        # Función para obtener el punto inicial de una geometría, sea LineString o MultiLineString
        def get_start_point(geom):
            if isinstance(geom, LineString):
                # Si es un LineString, simplemente tomamos el primer punto
        ...
        condicion = preprecondicion[~((preprecondicion['nomvtotal_1'] != preprecondicion['nomvtotal_2']) & (preprecondicion['cod_mun_1'] != preprecondicion['cod_mun_2']))].copy()####
        ...
        # Realizar el merge entre mv_gtm y condicion
        mv_gtm_actualizado = mv_gtm.merge(condicion[['id_1', 'incidencia_1']], left_on='id', right_on='id_1', how='left')
        # Actualizar la columna 'incidencia' solo donde 'incidencia_1' tenga valores
        mv_gtm_actualizado['incidencia'] = mv_gtm_actualizado['incidencia_1'].combine_first(mv_gtm_actualizado['incidencia'])
        # Eliminar las columnas auxiliares ('id_1' y 'incidencia_1') si ya no las necesitas
        mv_gtm_actualizado = mv_gtm_actualizado.drop(columns=['id_1', 'incidencia_1'])
        # Guardar los cambios en el DataFrame original
        mv_gtm = mv_gtm_actualizado.copy()
    ...
    if key == 7: 
        ...
        #MARCAR 
        cond1 = mv_gtm.index.isin(saltos_gen)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[7\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 7. Salto generadora"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "7. Salto generadora"
    ...
    if key == 8:
        ...
        cond1 = mv_gtm.index.isin(errores_apuntamiento)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[8\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 8. Error apuntamiento"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "8. Error apuntamiento"
    ...
    if key == 9 :
        ...
        filtro = placas_gtm[(placas_gtm['cod_mun']==texto_filtro)].dropna(subset=['nomvtotal','generadora'])
        resultados=[]
        ...
        for row in resultados_df.itertuples(index=False):
            if ((placas_gtm.at[row[0], 'nomvtotal']==copia_malla.at[row[1], 'nomvtotal']) and (placas_gtm.at[row[0], 'generadora'] in copia_malla.at[row[1], 'list_generadora'])):
                combo.append([row[1],row[0],placas_gtm.at[row[0],'placa']])
            elif ((placas_gtm.at[row[0], 'nomvtotal']==copia_malla.at[row[2], 'nomvtotal']) and (placas_gtm.at[row[0], 'generadora'] in copia_malla.at[row[2], 'list_generadora'])):
                combo.append([row[2],row[0],placas_gtm.at[row[0],'placa']])
        ...
        for row in impar_fin.itertuples(index = False):
            az_mv = calcular_azimuth(get_start_point(mv_gtm.loc[row[0],'geom']),get_end_point(mv_gtm.loc[row[0],'geom']))
            az_mv = radians((degrees(az_mv)+360)%360)
            az_p = calcular_azimuth(placas_gtm.loc[row[2],'geom'],placas_gtm.loc[row[4],'geom'])
        ...
        for row in par_fin.itertuples(index = False):
            ...
            if abs(az_mv-az_p)>radians(100):
                mala_orientacion.add(row[0])
        #MARCAR 
        cond1 = mv_gtm.index.isin(mala_orientacion)
        cond2 = mv_gtm['incidencia'].isna()
        cond3 = mv_gtm['exclusion'].str.contains(r'\[9\]', na=False)
        mv_gtm.loc[cond1 & ~cond2 & ~cond3, 'incidencia'] = mv_gtm['incidencia'] + " - 9. Invertir vector"
        mv_gtm.loc[cond1 & cond2 & ~cond3, 'incidencia'] = "9. Invertir vector"

        
    if key ==10 :
        #10 RANGO

        agrupados = mv_gtm[(mv_gtm['cod_mun']==texto_filtro) & (~mv_gtm['nomvia'].str.contains('S/N', na=False))][['cod_mun','tipovia','nomvia','generadora']].dropna().drop_duplicates()
        copia_placas = placas_gtm[(placas_gtm['cod_mun']==texto_filtro) & (~placas_gtm['nomvia'].str.contains('S/N', na=False)) & (placas_gtm['atipico'].isna())][['cod_mun','tipovia','nomvia','generadora','placa']].dropna().set_index(['cod_mun','tipovia','nomvia','generadora']).sort_index()
        sin_ran = set()
        sin_par = set()
        sin_impar = set()
        for row in agrupados.itertuples():
            ...
            if len(pares)>0:
                mv_gtm.at[row[0],'rangopar']=f"{pares[0]}, {pares[-1]}"
            else:
                sin_par.add(row[0])
            if len(impares)>0:
                mv_gtm.at[row[0],'rangoimpar']=f"{impares[0]},{impares[-1]}"
            else:
                sin_impar.add(row[0])
        #MARCAR 
        cond1 = mv_gtm.index.isin(sin_ran|sin_par)
        mv_gtm.loc[cond1 , 'rangopar'] = "Sin rango par"

        #MARCAR 
        cond1 = mv_gtm.index.isin(sin_ran|sin_impar)
        mv_gtm.loc[cond1 , 'rangoimpar'] = "Sin rango impar"
    
    return mv_gtm
    
def Gtm_validar_mv(engine, mavvial, placas, texto_filtro, seleccionados):
    
    campos_1 = 'id,geom,nom_dep,nom_mun,tipovia,nomvia,nomvtotal,atipico'
    campos_2 = 'id,geom,id_capa,atipico'
    campos_3 = 'id,geom,cod_dep,cod_mun,nomvia,generadora,atipico'
    campos_4 = 'id,geom,cod_dep,cod_mun,generadora,atipico'
    campos_5 = 'id,geom,cod_dep,cod_mun,cod_zona,nomvtotal,generadora,costado,atipico'  
    campos_6 = 'id,geom,cod_dep,cod_mun,nomvtotal,atipico'
    campos_7 = 'id,geom,cod_dep,cod_mun,nomvtotal,generadora,atipico'   
    campos_8 = 'id,geom,cod_dep,cod_mun,nomvtotal,generadora,atipico'
    campos_9 = 'id,geom,cod_dep,cod_mun,tipovia,nomvia,nomvtotal,generadora,atipico'
    campos_10 = 'id,geom,cod_dep,cod_mun,tipovia,nomvia,generadora,atipico'
    ...
    sql = f"SELECT {final_fields} FROM {mavvial} WHERE cod_mun = '{texto_filtro}';"
    mv_gtm = gpd.read_postgis(sql, engine, geom_col='geom',crs= 'EPSG:4326')
    mv_gtm['incidencia'] = None   
    mv_gtm['rangopar'] = None  
    mv_gtm['rangoimpar'] = None  
    mv_gtm['exclusion'] = mv_gtm['atipico'].apply(
    ...
    if (set(seleccionados) - {1,2,3,4,5,6,7,8}) != set():#Que no
        sql=f"SELECT id,geom,cod_dep,cod_mun,tipovia,nomvia,nomvtotal,generadora,placa,atipico FROM {placas} WHERE cod_mun = '{texto_filtro}';"
    ...
    for key in seleccionados:
        mv_gtm = Gtm_mv_incidencia( mv_gtm, placas_gtm, texto_filtro, key,idx)
    
    df_temporal = mv_gtm[['id', 'incidencia', 'rangopar', 'rangoimpar']]
